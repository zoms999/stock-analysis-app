...
-- New Table for View Deduplication
CREATE TABLE IF NOT EXISTS public.post_view_logs (
    id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    user_id UUID NOT NULL, -- REFERENCES profiles(id) usually, but keeping it loose to avoid dependency issues in this script if profiles not defined here
    post_id UUID NOT NULL, -- REFERENCES posts(id)
    viewed_at TIMESTAMPTZ DEFAULT NOW(),
    
    -- Ensure unique view log per user per post per day
    -- We use a date-truncated unique index or just store date? 
    -- Let's use a unique constraint on (user_id, post_id, date(viewed_at)).
    -- Note: date(viewed_at) is immutable.
    CONSTRAINT unique_daily_view UNIQUE (user_id, post_id, viewed_at) -- This is tricky with timestamp. 
    -- Better approach: Add a date column 'view_date' and use that for constraint.
);

-- Add view_date column if not exists (for idempotency)
ALTER TABLE public.post_view_logs 
ADD COLUMN IF NOT EXISTS view_date DATE DEFAULT CURRENT_DATE;

-- Add unique constraint if not exists
DO $$ 
BEGIN 
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'post_view_logs_unique_daily') THEN 
        ALTER TABLE public.post_view_logs 
        ADD CONSTRAINT post_view_logs_unique_daily UNIQUE (user_id, post_id, view_date);
    END IF; 
END $$;
...
-- Atomic View Consumption RPC (Updated for Deduplication)
